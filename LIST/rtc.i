...
#line 3 "..\\..\\SYSTEM\\SRC\\rtc.c"
#line 4 "..\\..\\SYSTEM\\SRC\\rtc.c"
...
ErrorStatus rtc_init(void)
{
	...
	if (0x87654321 == RTC_ReadBackupRegister(((uint32_t)0x00000000)))
	{
		return SUCCESS;
	}
	...
	while (!(((RCC_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x3800))->BDCR & 0x2) && (count < 0xFFFFFFUL))
	{
		count++;
	}
	if (count == 0xFFFFFFUL)  
  ...
  ((RTC_TypeDef *) (((uint32_t)0x40000000) + 0x2800))->WPR = 0x53;
	RTC_WriteBackupRegister(((uint32_t)0x00000000), 0x87654321);
...
ErrorStatus rtc_set_time(uint32_t hour, uint32_t min, uint32_t sec)
{
	...
	return RTC_SetTime(((uint32_t)0x000000000), &RTC_TimeStruct);
}
...
ErrorStatus rtc_set_date(uint32_t year, uint32_t mon, uint32_t date, uint32_t weekday)
{
	...
	return RTC_SetDate(((uint32_t)0x000000000), &RTC_DateStruct);
}
...
void rtc_print_date_time(void)
{
	...
	RTC_GetTime(((uint32_t)0x000000000), &time);
	RTC_GetDate(((uint32_t)0x000000000), &date);
	...
	uint8_t RTC_Hours, uint8_t RTC_Minutes, uint8_t RTC_Seconds)
{
	...
	RTC_SetAlarm(((uint32_t)0x000000000), RTC_Alarm, &RTC_AlarmStruct);
	...
	{
		RTC_ClearFlag(((uint32_t)0x00000200));
	}
...
void rtc_set_alarm_int(uint32_t RTC_Alarm)
{
	...
	EXTI_InitStruct.EXTI_Line = ((uint32_t)0x20000);
	...
	else   
	{
		...
		RTC_ITConfig(((uint32_t)0x00002000), ENABLE);
		
		RTC_ClearITPendingBit(((uint32_t)0x00002000));
	}
}
...
void rtc_set_wakeup_int(void)
{
	...
	EXTI_InitStruct.EXTI_Line = ((uint32_t)0x00400000);
	EXTI_InitStruct.EXTI_LineCmd = ENABLE;
	EXTI_InitStruct.EXTI_Mode = EXTI_Mode_Interrupt;
	EXTI_InitStruct.EXTI_Trigger = EXTI_Trigger_Rising;
	EXTI_Init(&EXTI_InitStruct);
...
{
	if (SET == RTC_GetITStatus(((uint32_t)0x00001000)))
	{
		alarma_flag = 1;
	...
	if (SET == RTC_GetITStatus(((uint32_t)0x00002000)))
	{
		alarmb_flag = 1;
