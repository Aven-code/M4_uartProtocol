...
#line 16 "..\\SYSTEM\\SRC\\protocol.c"

extern Func_Code receiveMsgType[20];
...
int scanfData() 
{
    ...
    if (!rece_flag) 
    {
        receiveDataArray[i] =  USART_ReceiveData(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1000))); 

        if (i == 0 && receiveDataArray[0] != 0xee)
        {
            i = 0;
            rece_flag = 0;                            
            memset(receiveDataArray, 0, 256); 
            return -1;
        }
...
int parsingAarry()
{
    ...
    if (!isRealCheckCode(receiveDataArray, len)) 
        return 0;                            
    ...
    for (i = 0; i < 20; i++)
    {
        if (receiveMsgType[i].funcode == src_funcode) 
        {
            receiveMsgType[i].fun_ptr(); 
            return 1;
        }
    }
    return 0;
