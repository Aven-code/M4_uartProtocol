...
void setChannel(void)
{
    ...
    u8 argType = receiveDataArray[8]; 

    u8 ch = receiveDataArray[7]; 
    ch--;                        
    ...
    else if (argType >= 1 && argType <= 31) 
    {
        argType = 1;

        allConfig.CH[ch].eq->agin = receiveDataArray[9];                                      
        allConfig.CH[ch].eq->Center_fre = (receiveDataArray[11] << 8) | receiveDataArray[10]; 
        allConfig.CH[ch].eq->valueQ = (receiveDataArray[13] << 8) | receiveDataArray[12];     
    }
    else if (argType == 32) 
    {
        argType = 2;
        allConfig.CH[ch].passA.low.type = receiveDataArray[9];                                  
        allConfig.CH[ch].passA.low.freque = (receiveDataArray[11] << 8) | receiveDataArray[10]; 
    ...
    else if (argType == 33) 
    {
        ...
        allConfig.CH[ch].passA.hign.freque = (receiveDataArray[11] << 8) | receiveDataArray[10]; 
    ...
    else if (argType == 35) 
    {
        ...
        allConfig.CH[ch].passA.value = receiveDataArray[10]; 
    }
    else if (argType == 36) 
    {
        argType = 6;
...
void setTotalVolume(void)
{
    ...
    allConfig.volume = receiveDataArray[7]; 
...
void setResetBalance(void)
{
    ...
    u8 j, ch = receiveDataArray[7]; 
    ch--;
    if (receiveDataArray[8] == 0x01) 
    {
        for (j = 0; j < 31; j++)
        {
            allConfig.CH[ch].eq[j].agin = 0x00;       
            allConfig.CH[ch].eq[j].valueQ = 0x02;     
            allConfig.CH[ch].eq[j].Center_fre = 0x32; 
        }

        sendResetBalance(0x01, receiveDataArray[7], receiveDataArray[4]);
    }
    else if (receiveDataArray[8] == 0x02) 
    {
...
void BluetoothMatch(void)
{
    ...
    info[4] = receiveDataArray[4];               
    info[11] = generateCheckCode(info, info[3]); 
    for (i = 0; i < 12; i++)
    {
...
void sendChannel(u8 flag, u8 sn_code)
{
    ...
    reply[8] = receiveDataArray[8];           
		reply[9] = flag;
    reply[10] = generateCheckCode(reply, 10); 
...
void sendTotalVolume(u8 sn_code)
{
    u8 i, reply[9] = {0xee, 0xee, 0x00, 0x09, 0x00, 0x18, 0x03, 0x00, 0x00};
		reply[4] = sn_code; 
    reply[7] = allConfig.volume;            
    reply[8] = generateCheckCode(reply, 8); 
...
void sendResetBalance(u8 type, u8 ch, u8 sn_code)
{
    u8 i, reply[10] = {0xee, 0xee, 0x00, 0x0A, 0x00, 0x18, 0x04, 0x00, 0x00, 0x00};
		...
		reply[7] = ch;
		reply[8] = type; 		
    reply[9] = generateCheckCode(reply, 9); 
...
void sendDelayCtr(u8 unit,u16 value,u8 ch, u8 sn_code)
{
    u8 i, reply[12] = {0xee, 0xee, 0x00, 0x0C, 0x00, 0x18, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00};
		...
		reply[7] = ch; 
		reply[8] = value & 0xff;   
    reply[9] = (value>>8) & 0xff; 
		reply[10] = unit; 
    reply[11] = generateCheckCode(reply, 11); 
...
void initApp(void)
{
    ...
    for (i = 1; i < 13; i++)
        sendDelayCtr(0,allConfig.CH[i].ms,i, 0); 
}
